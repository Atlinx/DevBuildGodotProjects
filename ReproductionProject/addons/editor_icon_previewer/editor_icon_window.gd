...
func _ready():
	...
	icon_preview.minimum_size = Vector2(MAX_ICON_SIZE, MAX_ICON_SIZE)
...
func add_icon(p_icon, p_name):
	...
	icon.texture = p_icon
	icon.minimum_size = Vector2(icon_size, icon_size)
	icon.hint_tooltip = p_name
	icon.name = p_name

	icon.gui_input.connect(_icon_gui_input.bind(icon))
...
func _icon_gui_input(event, icon):
	...
	if event is InputEventMouseButton and event.pressed:
		if event.button_index == MOUSE_BUTTON_LEFT:
			...
			DisplayServer.clipboard_set(icon.hint_tooltip)
...
func _update_icons():
	...
	for idx in previews_container.get_child_count():
		...
		icon.minimum_size = Vector2(icon_size, icon_size)
		icon.size = icon.minimum_size

	var sep = previews_container.get_theme_constant('hseparation')
